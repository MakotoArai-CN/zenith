...
fn resolveDiskPath(path: []const u8) []const u8 {
    var dir = compat.openDirCwd(path) catch {
...
fn benchSeqWrite(allocator: std.mem.Allocator, path: []const u8, size_bytes: usize, duration_secs: f64) !f64 {
    ...
    while (global.elapsed() < duration_secs) {
        const file = compat.createFileCwd(test_path, true) catch return 0;
        defer compat.fileClose(file);
        defer compat.deleteFileCwd(test_path) catch {};

        const t = timer_mod.Timer.start();
        var written: usize = 0;
        while (written < size_bytes) {
            const to_write = @min(BLOCK_SIZE, size_bytes - written);
            _ = compat.fileWrite(file, block[0..to_write]) catch break;
            written += to_write;
        }
...
fn benchSeqRead(allocator: std.mem.Allocator, path: []const u8, size_bytes: usize, duration_secs: f64) !f64 {
    ...
    {
        const wf = compat.createFileCwd(test_path, true) catch return 0;
        defer compat.fileClose(wf);
        var written: usize = 0;
        while (written < size_bytes) {
            const to_write = @min(BLOCK_SIZE, size_bytes - written);
            _ = compat.fileWrite(wf, write_block[0..to_write]) catch break;
            written += to_write;
        }
    }
    defer compat.deleteFileCwd(test_path) catch {};
    ...
    while (global.elapsed() < duration_secs) {
        const file = compat.openFileCwd(test_path) catch return 0;
        defer compat.fileClose(file);

        const t = timer_mod.Timer.start();
        var total_read: usize = 0;
        while (true) {
            const bytes_read = compat.fileRead(file, read_block) catch break;
...
fn benchRandWrite(allocator: std.mem.Allocator, path: []const u8, duration_secs: f64) !f64 {
    ...
    while (global.elapsed() < duration_secs) {
        ...
        @memset(fill, 0);
        _ = compat.fileWrite(file, fill) catch return 0;
        ...
        while (ops < RANDOM_OPS_COUNT) : (ops += 1) {
            ...
            compat.fileSeekTo(file, @intCast(offset)) catch break;
            _ = compat.fileWrite(file, block) catch break;
        }
...
fn benchRandRead(allocator: std.mem.Allocator, path: []const u8, size_bytes: usize, duration_secs: f64) !f64 {
    ...
    while (global.elapsed() < duration_secs) {
        ...
        while (ops < RANDOM_OPS_COUNT) : (ops += 1) {
            const offset = random.intRangeAtMost(usize, 0, max_blocks - 1) * RANDOM_BLOCK_SIZE;
            compat.fileSeekTo(file, @intCast(offset)) catch break;
            _ = compat.fileRead(file, read_block) catch break;
        }
        std.mem.doNotOptimizeAway(read_block.ptr);
